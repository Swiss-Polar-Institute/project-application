...
class CallList(TestCase):
    ...
    def test_load_funding_instrument_add(self):
        login = self._client.login(username='unittest_management', password='12345')
        self.assertTrue(login)

        response = self._client.get(reverse('logged-funding-instrument-add'))
        self.assertEqual(response.status_code, 200)

    def test_load_funding_instruments_list(self):
        response = self._client.get(reverse('logged-funding-instrument-list'))
        self.assertEqual(response.status_code, 200)

    def test_load_funding_instrument_update_get(self):
        response = self._client.get(reverse('logged-funding-instrument-update', kwargs={'pk': self._funding_instrument.id}))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self._funding_instrument.long_name)

    def test_load_funding_instrument_detail(self):
        response = self._client.get(reverse('logged-funding-instrument-detail', kwargs={'pk': self._funding_instrument.id}))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self._funding_instrument.long_name)
