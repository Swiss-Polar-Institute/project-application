...
class ValidIfEmpty:
    def __init__(self, fields_allowed_empty, basic_fields, form):
        ...
        self._base_class = form.__class__.__bases__[0]

    def update_required(self, fields):
        # Fields that are allowed empty are not required
        for field_allowed_empty in self._fields_allowed_empty:
            fields[field_allowed_empty].required = False
    ...
    def _is_empty(self):
        ...
        for field_name in self._form.fields:
            ...
            if self._form.prefix is not None:
                field_name_in_data = f'{self._form.prefix}-{field_name}'
            else:
                field_name_in_data = field_name

            if self._form.data.get(field_name_in_data, '') != '' or self._form.cleaned_data.get(field_name) is not None:
                return False
    ...
    def save(self, *args, **kwargs):
        ...
        """
        if self._is_empty():
            if self._form.instance.id:
                self._form.instance.delete()
