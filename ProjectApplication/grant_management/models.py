...
def carbon_emission_file_rename(instance, filename):
...
class CarbonEmission(CreateModifyOn):
    ...
    def __str__(self):
        return f'{self.file} ({self.estimate_carbon_emission}) ({self.effective_carbon_emission})'

    def save(self, *args, **kwargs):
        logger.info(
            f"Saving CarbonEmission: Project={self.project}, File={self.file}, "
            f"Estimate={self.estimate_carbon_emission}, Effective={self.effective_carbon_emission}"
        )
        super().save(*args, **kwargs)
